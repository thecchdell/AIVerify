...
class Plugin(IAlgorithm):
    ...
    The Plugin(Bias Algo) class specifies methods in generating results for algorithm
    """

    # Some information on plugin
    _name: str = "Bias Algo"
    _description: str = "Calculation for bias in dataset"
    _version: str = "0.1.0"
    ...
    def generate(self) -> None:
        ...
        analysis = [stats.kruskal(*[group[target_variable_name].values for name, group in self._data.groupby(test_variable_name)]) for test_variable_name in test_variable_names]
        analysis_df = pd.DataFrame(analysis, columns = ['statistic', 'pvalue'])
